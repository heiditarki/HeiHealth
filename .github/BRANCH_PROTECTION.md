...
After a PR is merged to `main` or `master`:

- **Backend**: Automatically deploys to Fly.io via `.github/workflows/deploy.yml`
- **Frontend**: Automatically deploys to Vercel (if Vercel is connected to the GitHub repository)

Make sure to set the `FLY_API_TOKEN` secret in GitHub repository settings (Settings → Secrets and variables → Actions) for backend deployment to work.
